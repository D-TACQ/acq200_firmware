...
function fftanalysis2( nsamples, frequency, filename )
#nsamples=4096;
#frequency=400;
binsize=0.0;
resfile="fftout";
...
#logamplitude=20*real(log10(amplitude/1130));
logamplitude=20*real(log10(amplitude/9040));
...
nyquist=fwrite( myfile, logamplitudeb, "float" );
fclose( myfile );
...
printf("SNR = %3.2fdB SINAD = %3.2fdB THD = %3.2fdB SFDR = %3.2fdBFS \n",SNR,SINAD,THD,SFDR);

#now draw the graph
titletext1 = sprintf("%s, Fundamental at %.3fkHz, Amplitude %.3fdB, using a %d point FFT",filename,fund_frequency,logfund_amp,nsamples);
titletext2 = sprintf("SNR=%3.2fdB SINAD=%3.2fdB THD=%3.2fdB SFDR=%3.2fdB",SNR,SINAD,THD,SFDR);
titletext3 = sprintf("Board Test Data, Not To Be Used Without Permission, Copyright D-TACQ Solutions 2004");
titlecommand = strcat("gset title \"",titletext1,"\\","\\n ",titletext2,"\\","\\n ",titletext3,"\"");
eval(titlecommand);
gset grid;
gset timestamp;
...
eval(outfilecommand);
axis ([1,shotfreq,-140,0]);
#axis ([1,nyquist,-140,0]);
ylabel ("Amplitude (dB)");
xlabel ("Frequency (kHz)");
#plot [1:nyquist] [-140:0] (binfrequency ,logamplitude);
plot (shortbin ,shortlog);
#plot (binfrequency ,logamplitude);
#gplot [1:nsamples/2] [-140:0] logamplitude;
gset output 
endfunction
